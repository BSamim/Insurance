...
$interestedVendorCount = 0;
if($Data){
foreach ($Data as $row) {
  $interestedVendorCount += 1;
}
}
//Calculate Active Quotes
$queryToGetActiveQuotes = "SELECT * FROM quotes WHERE quote_status = 'active'";
$active_quotes = db::getRecords($queryToGetActiveQuotes);
$activeQuotesCount = 0;
if($active_quotes){
foreach ($active_quotes as $acewd) {
  $activeQuotesCount += 1;
}
}
//Calculate Inactive Quotes
$queryToGetInactiveQuotes = "SELECT * FROM quotes WHERE NOT quote_status = 'active'";
$inactive_quotes = db::getRecords($queryToGetInactiveQuotes);
$inactiveQuotesCount = 0;
if ($inactive_quotes) {
  foreach ($inactive_quotes as $acsewd) {
    $inactiveQuotesCount += 1;
  }
}
?>
  ...
  <div class="utf-dashboard-content-inner-aera">
    ...
    <div class="utf-funfacts-container-aera">
      ...
      <div class="fun-fact" data-fun-fact-color="#f02727">
        ...
        <div class="fun-fact-text">
          <h4><?php echo $inactiveQuotesCount ?></h4>
          <span>Inactive RFQ</span>
        </div>
      </div>
    </div>
